...
namespace chat
{
	namespace
	{
		...
		{
			auto hidden_for_callback = false;
			...
			for (const auto& callback : say_callbacks)
			{
				...
				if (result.is<int>() && !hidden_for_callback)
				{
					hidden_for_callback = result.as<int>() == 0;
				}
			}

			std::string text(chatText);

			auto hidden_for_notify = text.front() == '/';

			game::add(hidden_for_notify);

			game::add(ent);

			if (hidden_for_notify)
			{
				text.erase(0, 1);
			}
			game::add(text.data());

			const auto notify_name = game::SL_GetString("say", 0);
			game::Scr_NotifyId(game::SCRIPTINSTANCE_SERVER, 0, *game::levelEntityId, notify_name, 3);

			if (!hidden_for_callback && !hidden_for_notify)
			{

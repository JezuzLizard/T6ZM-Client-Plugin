	...
	public:
		void post_unpack() override
		{
			...
			gsc::function::add("jsondump", [](const gsc::function_args& args)
			{
				...
				if (!utils::io::is_file_path_valid(scriptdata_folder, file))
				{
				...
				std::string full_path(scriptdata_folder + "/"s + file);
				...
				dumped_objects = {};
				return utils::io::write_file(full_path, gsc_to_json(value, print_id).dump(indent));
			});
		}
	};
}
